...
The SQL agent lab builds an agent that turns "find gaming influencers under $500" into a SQL query. If you skipped it, no problem — the branch you clone below already has that agent working. That agent only **reads** — worst case, it returns the wrong rows and the user shrugs.
