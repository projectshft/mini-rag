# Day 27 — Assignment: RAG Agent


> **Today:** ship it. Your Week 4 assignment is due — the full retrieval stack in one file (preprocessing, reranking, score threshold), plus a video where you explain how you evaluate retrieval quality. This is the Feynman moment for everything you built this week.

## What your RAG agent must do

Quick recap of the week. Your `ragAgent` in [`app/agents/rag.ts`](https://github.com/projectshft/mini-rag/blob/student-todo-exercises/app/agents/rag.ts) should run the full pipeline you built this week:
...
Reranking and score thresholding are part of this assignment too — everything you built this week lands in `rag.ts` together.

## Assignment

### Video (4–5 minutes)

Record yourself explaining **how you evaluate retrieval quality**, Feynman-style — as if to a sharp colleague who's never built RAG. Address these five questions:
...
4. **The two-stage pattern** — why aren't cosine similarity's top hits always the
   best answers, and what does the reranker see that the embedding comparison
   can't? Explain your over-fetch number and your threshold, and what each costs.
5. **Metrics** — what would you track in production to monitor retrieval quality? (Yesterday's LangSmith setup should give you ideas.)
...
Everything this week goes into one file. Complete the TODOs in the RAG agent so
it retrieves and answers, then extend it:

1. **Query preprocessing** — expand abbreviations, strip filler words, as described above
2. **Reranking** — over-fetch on the first stage, then rerank with
   `pineconeClient.inference.rerank('bge-reranker-v2-m3', ...)` and build your
   context from the reranked results
3. **Score threshold** — drop results below a minimum you pick, and return a
   graceful "I don't have enough information" when nothing clears it

That's Phases 1 through 4 of the TODO block in the file. The threshold is the
one that separates a demo from something you'd put in front of a user: a
...
**A video that recites definitions.** "Chunks can be too big or too small" earns no points. "My 1000-char chunks kept splitting code examples mid-function, so answers about `useState` came back half-baked — here's the trace" is what this video should sound like.
...
Then in `ragAgent`: `const query = preprocessQuery(request.query);` and embed `query`. Being a pure function, you can demo it in isolation and unit-test it later (testing week is coming up next).
...
---
I'm submitting a RAG agent for a course assignment. It lives in app/agents/rag.ts and does: query preprocessing (abbreviation expansion, casing normalization, filler-word stripping) -> embedding with text-embedding-3-small -> Pinecone query (over-fetched topK, includeMetadata) -> rerank with bge-reranker-v2-m3 -> score threshold with a graceful "I don't know" -> context extraction -> grounded system prompt (original + refined query + context + "say if insufficient") -> streamText with gpt-4o.
...
---
I'm about to record a 4–5 minute Feynman-style video on evaluating retrieval quality in my RAG system. I must cover: chunk sizing symptoms, retrieval accuracy (what "wrong" looks like), choosing similarity thresholds, the two-stage retrieval pattern (why cosine's top hits aren't always best, what the reranker sees that embeddings can't), and production metrics.

Run a rehearsal: I'll deliver my explanation as text. Time-check it (roughly 150 words per spoken minute), then grade each of the five topics on (a) did I use a SPECIFIC example from my own implementation, and (b) would a smart non-RAG engineer follow it. Ask me the two follow-up questions a skeptical reviewer would ask. If any section was generic textbook-talk, make me redo just that section with a concrete example before you sign off.
```
