...
Many production systems use both — and the router you built earlier is exactly the piece that decides which retrieval method fits the query:
...
This is the same structured-outputs technique doing a new job: instead of the LLM writing SQL strings (fragile, dangerous), it extracts **typed parameters** and your code builds the query:
...
**Key insight**: with Prisma, you're building queries with a type-safe API, not concatenating strings. The database receives the query structure and values separately. This matters double in an LLM app — the "user input" flowing into your query might have been generated by a model processing untrusted text. (Tomorrow's security lesson goes deep on this class of problem.)
...
**This lab is optional and ungraded.** It's self-contained, it makes a strong portfolio piece, and it's the clearest proof that "RAG" never meant "vectors." Do it now if the idea grabs you, or come back to it during capstone week.
